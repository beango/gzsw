...
{
    public class StatDownController : BaseController
    {
        const string SAVE_PATH = "/Uploads/DownSWF/";
        const string HTTP_URI = "/Uploads/DownSWF/";
        bool OVERWRITEFILE = false;
        bool INTELLIGENTFILENAMING = true;
        string FILESUFFIXFORMAT = "TIMESTAMP";// // value can be either 'TIMESTAMP' or 'RANDOM'
        const string MIMETYPES = "pdf=application/pdf;jpg=image/jpeg;jpeg=image/jpeg;gif=image/gif;png=image/png";
        const string EXTENSIONS = "pdf=pdf;jpg=jpg;jpeg=jpg;gif=gif;png=png";
        const string DEFAULTPARAMS = "exportfilename=FusionCharts;exportformat=PDF;exportaction=download;exporttargetwindow=_self";
        string notices = "";
        ...
        {
            //return buildResponse(parseExportedStatus(filename, meta, msg));
        ...
        private Hashtable setupServer(Hashtable exportParams)
        {
            ...
            if (!System.IO.File.Exists(path + retServerStatus["filepath"].ToString()))
            {
                ...
                if (dirWritable)
                {
                    ...
                    fos = System.IO.File.Open(path + retServerStatus["filepath"].ToString(), FileMode.Create, FileAccess.Write);
        ...
        private Hashtable setupDownload(Hashtable exportParams)
        {
            //get export filename
            string exportFile = exportParams["exportfilename"].ToString();
            exportFile = HttpUtility.UrlDecode(exportFile, Encoding.UTF8);
            //get extension
            string ext = getExtension(exportParams["exportformat"].ToString());
            //get mime type
