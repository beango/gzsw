...
{
    public class QueuingDetentionController : BaseController<CHK_HALL_STAT_M>
    {
        [Ninject.Inject]
        ...
        public ActionResult Index(string statMo, string orgId, int pageIndex = 1, int pageSize = 20)
        {
            ...
            var data = new CHK_HALL_STAT_M_DAL().GetQueuingDetentionList(pageIndex, pageSize, orgId, stat);
            return View(data);
        }
        ...
        public ActionResult Detail(string id, string staffId)
        {
            try
            {
                var model = CHK_HALL_STAT_M_DAL.GetQueuingDetention(Convert.ToInt32(id), staffId);
        ...
        public ActionResult Edit(string id, string staffId)
        {
            ...
            try
            {
                var item = CHK_HALL_STAT_M_DAL.GetQueuingDetention(Convert.ToInt32(id), staffId);

                var model = new QueuingDetentionViewModel()
                {
                    COR_OVERTIME_CNT = (int)item.COR_OVERTIME_CNT,
                    COR_VALID_QUEUE_CNT = (int)item.COR_VALID_QUEUE_CNT, 
