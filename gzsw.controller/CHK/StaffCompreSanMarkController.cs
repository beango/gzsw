...
namespace gzsw.controller.CHK
{
    ...
    ///     综合评议额外奖罚分
    /// </summary>
    public class StaffCompreSanMarkController : BaseController<CHK_STAFF_COMPRE_SAN_MARK>
    {
        [UserAuth("CHK_STAFF_COMPRE_SAN_MARK_VIW")]
        public ActionResult Index(string orgId,int? type, string staffNo, string staffName, int pageIndex = 1, int pageSize = 20)
        {
            ...
            var list = CHK_STAFF_COMPRE_SAN_MARK_DAL.GetList(orgId,type,staffNo, staffName, UserState.UserID, pageIndex, pageSize);
            return View(list);
        }
        ...
        [UserAuth("CHK_STAFF_COMPRE_SAN_MARK_ADD")]
        ...
        [HttpPost]
        [UserAuth("CHK_STAFF_COMPRE_SAN_MARK_ADD")]
        public ActionResult Create(StaffCompreSanMarkAddModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    var item = new CHK_STAFF_COMPRE_SAN_MARK()
                    {
                        ...
                        SCORE=model.SCORE,
                        STAT_MO = int.Parse(model.STAT_MO.Replace("-","")),
                    };
        ...
        [UserAuth("CHK_STAFF_COMPRE_SAN_MARK_EDT")]
        ...
        [HttpPost]
        [UserAuth("CHK_STAFF_COMPRE_SAN_MARK_EDT")]
        public ActionResult Edit(StaffCompreSanMarkModel model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [UserAuth("CHK_STAFF_COMPRE_SAN_MARK_DEL")]
        ...
        private StaffCompreSanMarkModel getModel(int id)
        {
            var item = CHK_STAFF_COMPRE_SAN_MARK_DAL.Get(id);
            var statmo = item.STAT_MO.ToString();
            statmo = statmo.Insert(4, "年") + "月";
            return new StaffCompreSanMarkModel()
            {
                SCORE = item.SCORE,
                ORG_ID = item.ORG_ID,
