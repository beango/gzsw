...
namespace gzsw.winservice.Job
{
    ...
    public class PRO_STAT_STAFF_QUEUE_BUSI_D_JOB : IJob
    {
        #region IJob 成员
        ...
        public void Execute(IJobExecutionContext context)
        {
            try
            {
                LogHelper.WriteLog("PRO_STAT_STAFF_QUEUE_BUSI_D_JOB");
                var db = gzswDB.GetInstance();

                model.SVR_TIM_EVENT e = db.SingleOrDefault<model.SVR_TIM_EVENT>("select * from SVR_TIM_EVENT where PROGRAM_METHOD='PRO_STAT_STAFF_QUEUE_BUSI_D'");
                if (null != e)
                {
            ...
            {
                LogHelper.ErrorLog("PRO_STAT_STAFF_QUEUE_BUSI_D_JOB.Execute", ex);
            }
        }
        ...
        private bool ExecuteProd(SVR_TIM_EVENT_LOG log)
        {
            try
            {
                ...
                db.Execute("exec PRO_STAT_STAFF_QUEUE_BUSI_D @0, @1", parlist.Select(obj => obj.PAR_VALUE).ToArray());
                if (null != par1)
                {
