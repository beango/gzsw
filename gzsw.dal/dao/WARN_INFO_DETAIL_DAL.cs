...
namespace gzsw.dal.dao
{
    public class WARN_INFO_DETAIL_DAL
    {
        ...
        {
            var sql = Sql.Builder.Append("select t2.* from dbo.WARN_INFO_DETAIL t1 join dbo.WARN_SENDINFO_DETAIL t2 on t1.WARN_INFO_DETAIL_ID=t2.WARN_INFO_DETAIL_ID where t1.STATE=1 and t2.CLI_READ_IND=0 and t2.CLI_SEND_STATE=0 and t2.USER_ID=@0 and DATEDIFF(dd,t1.CREATE_DTIME,GETDATE())=0", userid);
